...
def run(input_seqs, kmer_size=5, weighting="linear", threshold=0.1, count=10000):
    ...
    for i in range(ITERS):
        ...
        full_positive_matrix, full_negative_matrix = analysis.generate_truthtables(to_exclude)
...
if __name__ == '__main__':
    ...
    with mp.Pool(4, initializer=make_global, initargs=(allSeqs,)) as pool:
        ...
        pd.DataFrame.from_dict(dict(data)).to_csv(f"./results_downsampling/N{ITERS}_constrained.csv", sep=",", index=False)
